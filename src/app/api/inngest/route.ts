...
// Note: Automated implementation was removed in favor of manual Claude Code prompts.
// The following imports are kept for potential future use but prefixed with underscore:
 ...
 */
async function _analyzeRoleDecision(
  role: 'Reviewer' | 'QA',
  response: string
...
Keep your response focused on architecture validation. Be concise but thorough.`;
    ...
    // Instead of automated code generation, direct users to the dashboard
    // to get a copy-paste-ready prompt for Claude Code
    // This enables human oversight, local testing, and manual commits
    // =========================================================================

    const baseUrl = process.env.NEXTAUTH_URL || process.env.VERCEL_URL || 'http://localhost:3000';
    const dashboardUrl = `${baseUrl}/projects/${projectSlug}`;

    // Post instructions for manual implementation
    await step.run('post-manual-implementation-instructions', async () => {
      const workingDir = targetCodebase || projectSlug;

      return postAsRole('Implementer', channel, 'Ready for implementation', [
        {
          type: 'header',
          text: { type: 'plain_text', text: '🛠️ Ready for Implementation', emoji: true },
        },
        {
          type: 'section',
          text: {
            type: 'mrkdwn',
            text: `*${projectName}* Sprint ${sprintNumber} is ready for implementation.\n\nThe handoff document has been generated. Use the dashboard to get the Claude Code prompt.`,
          },
        },
        {
          type: 'section',
          fields: [
            { type: 'mrkdwn', text: `*Working Directory:*\n\`C:\\tpml-ai-team\\projects\\${workingDir}\`` },
            { type: 'mrkdwn', text: `*Sprint:*\n${sprintNumber}${sprintName ? ` - ${sprintName}` : ''}` },
          ],
        },
        {
          type: 'section',
          text: {
            type: 'mrkdwn',
            text: `*Next Steps:*\n1. Open the project dashboard\n2. Expand "Claude Code Prompt" section\n3. Copy the prompt\n4. Open terminal in the working directory\n5. Run \`claude\` and paste the prompt\n6. Review changes locally and test\n7. Commit when satisfied`,
          },
        },
        {
          type: 'actions',
          elements: [
            {
              type: 'button',
              text: { type: 'plain_text', text: '📋 Get Claude Code Prompt', emoji: true },
              style: 'primary',
              url: dashboardUrl,
            },
          ],
        },
        {
          type: 'context',
          elements: [
            { type: 'mrkdwn', text: `_Manual implementation mode - human oversight enabled_` },
          ],
        },
      ], kickoffMessage?.ts);
    });

    console.log(`[Kickoff] Project ${projectName} Sprint ${sprintNumber} ready for manual implementation`);
    ...
    return {
      ...
      mode: 'manual',
      dashboardUrl,
      message: 'Ready for manual implementation. Get the Claude Code prompt from the dashboard.',
      messageTs: kickoffMessage?.ts,
    };
  }
);

// Note: Automated implementation workflow was removed in favor of manual Claude Code prompts.
// Users now get a copy-paste-ready prompt from the dashboard for human-controlled implementation.
/**
